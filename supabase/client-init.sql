...
-- 默认餐馆设置（客户上线前需要修改）
insert into public.restaurant_settings (name_zh, name_en, name_el, ordering_enabled)
values ('新餐馆', 'New Restaurant', 'Νέο Εστιατόριο', true)
on conflict do nothing;
...
alter table public.restaurant_settings
  ...
  add column if not exists feature_flags jsonb not null default '{"csv_import":true,"ai_menu":true,"ai_image":true,"data_backup":true,"print_agent":true,"reservations":false}'::jsonb;
...
-- Restaurant reservations: public booking is only available through the two RPCs below.
...
create index if not exists idx_reservations_slot_status on public.reservations (reservation_date, reservation_time, status);
create index if not exists idx_reservations_created_at on public.reservations (created_at desc);
alter table public.reservations drop constraint if exists reservations_status_check;
alter table public.reservations add constraint reservations_status_check check (status in ('confirmed', 'arrived', 'completed', 'cancelled', 'no_show'));
drop trigger if exists set_reservation_settings_updated_at on public.reservation_settings;
create trigger set_reservation_settings_updated_at before update on public.reservation_settings for each row execute function public.set_updated_at();
...
insert into public.reservation_settings (singleton) select true where not exists (select 1 from public.reservation_settings);
...
create policy "reservation_settings_public_read" on public.reservation_settings for select to anon, authenticated using (true);
drop policy if exists "reservation_settings_staff_manage" on public.reservation_settings;
create policy "reservation_settings_staff_manage" on public.reservation_settings for all to authenticated using ((select private.is_staff())) with check ((select private.is_staff()));
drop policy if exists "reservations_staff_read" on public.reservations;
create policy "reservations_staff_read" on public.reservations for select to authenticated using ((select private.is_staff()));
drop policy if exists "reservations_staff_update" on public.reservations;
create policy "reservations_staff_update" on public.reservations for update to authenticated using ((select private.is_staff())) with check ((select private.is_staff()));
...
language plpgsql security definer set search_path = public, private
as $$
declare s public.reservation_settings%rowtype; local_now timestamp;
begin
  ...
  return query
  select slot_value::time, greatest(0, s.capacity_per_slot - coalesce((select sum(r.party_size)::integer from public.reservations r where r.reservation_date = p_date and r.reservation_time = slot_value::time and r.status in ('confirmed', 'arrived')), 0))
  from generate_series((p_date + s.open_time)::timestamp, (p_date + s.close_time)::timestamp - make_interval(mins => s.slot_interval_minutes), make_interval(mins => s.slot_interval_minutes)) as slot_value
  where slot_value >= local_now + make_interval(mins => s.minimum_notice_minutes)
  order by slot_value;
end;
$$;

create or replace function public.create_reservation(p_date date, p_time time, p_party_size integer, p_guest_name text, p_phone text, p_note text default null)
returns table (reservation_id uuid, reference_code text, reservation_status text)
language plpgsql security definer set search_path = public, private
as $$
declare s public.reservation_settings%rowtype; local_now timestamp; booked_capacity integer; version_no text; new_id uuid; new_code text;
begin
  select * into s from public.reservation_settings limit 1;
  if not found or not s.is_enabled then raise exception 'Reservations are not available.'; end if;
  local_now := now() at time zone s.timezone;
  if p_date < local_now::date or p_date > local_now::date + s.max_advance_days then raise exception 'Selected date is not available.'; end if;
  if p_party_size < 1 or p_party_size > s.max_party_size then raise exception 'Party size is not available.'; end if;
  if length(btrim(coalesce(p_guest_name, ''))) < 2 or length(btrim(p_guest_name)) > 120 then raise exception 'Please provide a valid name.'; end if;
  if length(btrim(coalesce(p_phone, ''))) < 5 or length(btrim(p_phone)) > 40 then raise exception 'Please provide a valid phone number.'; end if;
  if coalesce(length(p_note), 0) > 1000 then raise exception 'Note is too long.'; end if;
  if p_time < s.open_time or p_time >= s.close_time or extract(epoch from (p_time - s.open_time))::integer % (s.slot_interval_minutes * 60) <> 0 then raise exception 'Selected time is not available.'; end if;
  if (p_date + p_time)::timestamp < local_now + make_interval(mins => s.minimum_notice_minutes) then raise exception 'Selected time requires more notice.'; end if;
  perform pg_advisory_xact_lock(hashtext('reservation:' || p_date::text || ':' || p_time::text));
  select coalesce(sum(party_size), 0) into booked_capacity from public.reservations where reservation_date = p_date and reservation_time = p_time and status in ('confirmed', 'arrived');
  if booked_capacity + p_party_size > s.capacity_per_slot then raise exception 'Selected time is fully booked.'; end if;
  select public.current_legal_version() into version_no;
  new_id := gen_random_uuid(); new_code := 'RES-' || to_char(p_date, 'YYYYMMDD') || '-' || upper(substr(replace(new_id::text, '-', ''), 1, 6));
  insert into public.reservations (id, reference_code, reservation_date, reservation_time, party_size, guest_name, phone, note, legal_terms_version, privacy_policy_version, legal_accepted_at)
  values (new_id, new_code, p_date, p_time, p_party_size, btrim(p_guest_name), btrim(p_phone), nullif(btrim(coalesce(p_note, '')), ''), version_no, version_no, case when version_no is null then null else now() end);
