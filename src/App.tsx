...
function PublicShell() {
  ...
  useEffect(() => {
    let cancelled = false;
    void getPublicRestaurantSettings().then((next) => !cancelled && setSettings(next)).catch(() => !cancelled && setSettings(null));
  ...
  return <>
    <header className="site-header"><div className="site-header-inner">
      ...
      <nav className={`nav-links ${mobileNavOpen ? 'is-open' : ''}`} aria-label={t('nav.primary')}><NavLink to="/" end onClick={() => setMobileNavOpen(false)}>{t('nav.home')}</NavLink><NavLink to="/menu" onClick={() => setMobileNavOpen(false)}>{t('nav.menu')}</NavLink>{reservationsEnabled ? <NavLink to="/reservations" onClick={() => setMobileNavOpen(false)}>{lang === 'zh' ? '预订餐桌' : lang === 'en' ? 'Reserve a table' : 'Κράτηση'}</NavLink> : null}<NavLink to="/contact" onClick={() => setMobileNavOpen(false)}>{t('nav.contact')}</NavLink></nav>
      ...
      <div className="site-header-actions"><LanguageSwitch /></div>
    </div></header>
    <Suspense fallback={null}><Routes><Route path="/" element={<HomePage />} /><Route path="/menu" element={<MenuPage />} /><Route path="/reservations" element={reservationsEnabled ? <ReservationPage /> : <Navigate to="/" replace />} /><Route path="/privacy-policy" element={<LegalPage />} /><Route path="/terms-of-service" element={<LegalPage />} /><Route path="/cookie-policy" element={<LegalPage />} /><Route path="/contact" element={<LegalPage />} /><Route path="/cancellation-policy" element={<LegalPage />} /><Route path="/table/:qrToken" element={<TableOrderPage />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes></Suspense>
